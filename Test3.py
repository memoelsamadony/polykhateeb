...
SYSTEM_PROMPT_POLISHER = """ROLE: You are an expert editor.
TASK: Improve the flow and grammar of the provided text.
RULES:
1. Combine the sentences into a natural paragraph.
2. Fix phonetic errors (e.g., "victims" -> "desires").
3. Output ONLY the polished English text.
"""
...
def robust_ollama_call(payload, timeout=30, retries=2):
    """
    محاولة الاتصال بـ Ollama مع إعادة المحاولة في حالة الفشل أو انتهاء الوقت.
    """
    for attempt in range(retries + 1):
        try:
            ...
            log(f"[API] Timeout (Attempt {attempt+1}/{retries+1})... Retrying.")
        except Exception as e:
            log(f"[API] Error (Attempt {attempt+1}/{retries+1}): {e}")
...
def load_whisper_model(model_size, device, compute_type):
    log_gpu(f"Pre-Load Whisper ({model_size}, {compute_type})")
    try:
        gc.collect()
    ...
    def run_refinement():
        ...
        while True:
            try:
                ...
                target_text = job.get('target', '')
                context_text = job.get('context', '')
                batch_id = job.get('id', 0)
                log_file = job.get('log_file', None)
                
                if target_text:
                    prompt = f"Previous Context: {context_text}\n\nCurrent Text to Polish: {target_text}\n\nTask: Polish the 'Current Text' into a smooth paragraph."
                    
                    # Increased timeout to 60s for heavy batches
                    ...
                    if refined:
                        if "Chinese" not in refined and not re.search(r'[\u4e00-\u9fff]', refined):
                            ...
                            log(f"[REFINE] Batch {batch_id} Polished.")
                            if log_file:
                                append_to_file(log_file, f"\n[Batch {batch_id}]\n{refined}\n")
                        else:
                            log(f"[REFINE] Batch {batch_id} Rejected (Hallucination).")
                    else:
                        log(f"[REFINE] Batch {batch_id} Failed (Timeout/Error).")
...
def transcription_stream_thread(wav_path, config, event_q, stop_event, refine_input_q):
    ...
    with open(trans_log, "w", encoding="utf-8") as f: f.write("--- ARABIC TRANSCRIPTION LOG ---\n")
    with open(transl_log, "w", encoding="utf-8") as f: f.write("--- ENGLISH TRANSLATION LOG ---\n")
    with open(refined_log, "w", encoding="utf-8") as f: f.write("--- REFINED OUTPUT LOG ---\n")
        ...
        try:
            ...
            if chunk_ar and len(chunk_ar) > 2:
                ...
                if "إن الحمد لله" in chunk_ar:
                    ...
                    if not raw_tr:
                        raw_tr = "..." # Fallback on final failure
                    else:
                        is_valid, reason = validate_translation(chunk_ar, raw_tr)
                ...
                # === 4. Batching Logic ===
                refine_buffer.append(final_tr)
                if len(refine_buffer) >= config['refine_every']:
                    batch_counter += 1
                    text_to_refine = " ".join(refine_buffer)
                    
                    refine_input_q.put({
                        "id": batch_counter,
                        "target": text_to_refine,
                        "context": last_context,
                        "log_file": refined_log
                    })
                    
                    last_context = text_to_refine[-300:] 
                    refine_buffer = [] 
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        refine_every = st.slider("Refine Every N Chunks", 1, 5, 3)
        chunk_dur = st.slider("Chunk Duration (s)", 30, 90, 60)
    ...
    st.title("🕌 Khutbah AI: Robust Logger")
