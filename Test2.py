...
# Whisper decoding defaults (IMPROVED: reduced hallucination)
WHISPER_BEAM_DEFAULT = 5
WHISPER_BEAM_MAX = 8
WHISPER_BEST_OF_DEFAULT = 5
WHISPER_BEST_OF_MAX = 8

# CRITICAL: Add these new parameters to reduce hallucination
WHISPER_TEMPERATURE = 0.0  # Use greedy decoding

# Streaming chunking defaults (IMPROVED: better chunking)
CHUNK_SEC_DEFAULT = 40.0  # Reduced from 30 for more frequent updates
CHUNK_SEC_MAX = 60.0  # Reduced from 90
OVERLAP_SEC_DEFAULT = 2.0  # Increased from 1.0 for better context

DEFAULT_QWEN_REFINE_EVERY_CHUNKS = 5  # Increased from 3 to reduce load
QWEN_MODEL = "qwen2.5:7b-instruct"
QWEN_ENDPOINT = "http://localhost:11434/api/chat"

MT_MODEL = "qwen2.5:7b-instruct"  # Full name required
...
INITIAL_PROMPT_ISLAMIC = (
    "This is an Arabic Islamic lecture. Preserve Islamic terms faithfully: Allah, Qur'an, Sunnah, hadith, salah, zakah, sawm, Hajj, Umrah, tawhid, ummah, dua, khutbah, imam."
)
...
MAX_MT_CHARS = 700
QWEN_AR_TAIL = 3000  # Increased from 2200
QWEN_DRAFT_TAIL = 3000  # Increased from 2200
...
def ollama_translate_sentence(text_ar: str, target_lang: str) -> str:
    ...
    for attempt in range(3):
        ...
        try:
            ...
            log(f"[MT] Translation attempt {attempt+1} failed: {e}")
...
# ---------------- Qwen via Ollama (MAIN THREAD ONLY) - FIXED ----------------
...
def qwen_refine_translation(ar_tail: str, draft_tail: str, target_lang: str, model_name: str = QWEN_MODEL, qwen_log_path: Optional[str] = None) -> str:
    """FIXED: Improved Qwen refinement with better error handling and timeout."""
    ...
    for attempt in range(2):
        payload = {
            ...
            "options": {
                ...
                "num_gpu": 1,
                "num_predict": 4096,  # Increased to handle longer responses
                "repeat_penalty": 1.1,
            },
        }
        ...
        try:
            ...
            # CRITICAL FIX: Increased timeout for Qwen
            resp = requests.post(QWEN_ENDPOINT, json=payload, timeout=600)  # 10 minutes
            resp.raise_for_status()
            data = resp.json()
            ...
            if not text:
                log(f"[QWEN] Empty response, data keys: {data.keys()}")
                text = ""
            else:
                log(f"[QWEN] Response content:\n{text}")
                if qwen_log_path:
                    import datetime
                    timestamp = datetime.datetime.now().isoformat()
                    append_line(qwen_log_path, f"\n{'='*80}")
                    append_line(qwen_log_path, f"Timestamp: {timestamp}")
                    append_line(qwen_log_path, f"Attempt: {attempt+1}")
                    append_line(qwen_log_path, f"Model: {model_name}")
                    append_line(qwen_log_path, f"AR tail length: {len(ar_tail)}")
                    append_line(qwen_log_path, f"Draft tail length: {len(draft_tail)}")
                    append_line(qwen_log_path, f"{'='*80}")
                    append_line(qwen_log_path, f"RESPONSE:\n{text}")
                    append_line(qwen_log_path, f"{'='*80}\n")
            
            log(f"[QWEN] Received response length: {len(text)}")
            
            if validate_qwen(text) and len(text) > 50:  # Ensure meaningful response
                log(f"[QWEN] Success on attempt {attempt+1}")
                return text
            
            log(f"[QWEN] Output failed validation on attempt {attempt+1}: len={len(text)}, valid={validate_qwen(text)}")
            # Tighten system for retry
            system = system + "\n\nFINAL WARNING: Output ONLY the polished translation text. NO meta commentary."
        except requests.exceptions.Timeout:
            log(f"[QWEN] Timeout on attempt {attempt+1}")
            if attempt == 1:
                return draft_tail  # Return original draft on final timeout
        except Exception as e:
            log(f"[QWEN] Error during refine attempt {attempt+1}: {type(e).__name__}: {e}")
            if attempt == 1:
                return draft_tail  # Return original draft on final error
    
    # Fallback: return original draft if all attempts fail
    return draft_tail
...
def transcribe_file_once_with_model(model: WhisperModel, path: str, beam_size: int, best_of: int, initial_prompt: str) -> str:
    """IMPROVED: Added parameters to reduce hallucination."""
    segments, info = model.transcribe(
        ...
        vad_filter=True,
        vad_parameters=dict(
            min_silence_duration_ms=500,  # Increased from default 2000
            threshold=0.5,  # Default
        ),
        temperature=WHISPER_TEMPERATURE,  # CRITICAL: Greedy decoding
        language="ar",
        initial_prompt=None,  # CRITICAL: Don't pass English prompt to Arabic transcription
        condition_on_previous_text=True,
        word_timestamps=False,  # Disable for speed
    )
    
    # Collect all segment text
    result_text = [s.text for s in segments]
    ...
    try:
        with wave.open(wav_path, "rb") as wf:
            ...
            while not stop_event.is_set():
                ...
                try:
                    # IMPROVED: Use previous Arabic transcription as prompt (not English terms)
                    prompt_tail = rolling_prompt[-600:] if rolling_prompt else None  # Only Arabic context
                ...
                if chunk_text:
                    ...
                    rolling_prompt = full_text[-600:]  # Reduced from 800
                        ...
                        for sent in done_sents:
                            for piece in force_chunk(sent, MAX_MT_CHARS):
                                try:
                                    ...
                                    if translated:
                                        log(f"[MT] Translated: {piece[:80]} -> {translated[:80]}")
                                except Exception as e:
...
def ensure_state():
    ...
    # Placeholder containers for dynamic display
    if "transcript_container" not in st.session_state:
        st.session_state.transcript_container = None
    if "translation_container" not in st.session_state:
        st.session_state.translation_container = None
    if "refined_container" not in st.session_state:
        st.session_state.refined_container = None
    ...
    if "qwen_log_path" not in st.session_state:
        base_dir = Path(__file__).resolve().parent
        st.session_state.qwen_log_path = str(base_dir / f"qwen_refinement_{st.session_state.session_id}.txt")
    ensure_log_file(st.session_state.transcript_log_path)
    ensure_log_file(st.session_state.translation_log_path)
    ensure_log_file(st.session_state.qwen_log_path)
...
def app_ui():
    ...
    st.title("Realtime Transcribe & Translate (Fixed: Reduced hallucination + Qwen working)")
    ...
    with st.sidebar:
        ...
        model_size = st.selectbox(
            ...
            index=["tiny", "base", "small", "medium", "large-v2", "large-v3"].index(WHISPER_MODEL_SIZE),
        )
        ...
        overlap_sec = st.slider("Overlap (s)", 0.0, 5.0, OVERLAP_SEC_DEFAULT, 0.1)
    ...
    with col1:
        ...
        transcript_text = st.session_state.full_transcript or "(waiting for transcription...)"
        st.text_area(
            "Transcript",
            value=transcript_text,
            height=400,
            key=f"transcript_{st.session_state.chunk_total}",
            label_visibility="collapsed",
        )

    with col2:
        st.subheader(f"Translation (Local, {target_lang})")
        translation_text = st.session_state.full_translation_local or "(waiting for translation...)"
        st.text_area(
            "Translation",
            value=translation_text,
            height=400,
            key=f"translation_{st.session_state.chunk_total}",
            label_visibility="collapsed",
        )
    ...
    st.caption(
        ...
        f"extraction_done={st.session_state.extraction_done} | qwen_inflight={st.session_state.qwen_inflight}"
    )

    st.subheader("Refined Translation (Qwen via Ollama)")
    refined_text = st.session_state.full_translation_refined or "(Qwen refinement not yet run)"
    st.text_area(
        "Refined",
        value=refined_text,
        height=300,
        key=f"refined_{st.session_state.chunk_total}",
        label_visibility="collapsed",
    )
    ...
    if uploaded is not None:
        ...
        # Only display video if file exists
        if st.session_state.video_path and os.path.exists(st.session_state.video_path):
            st.video(st.session_state.video_path)
        else:
            st.warning("⚠️ Video file no longer available (may have been cleaned up)")
        ...
        if st.session_state.start_stream_request and st.session_state.extraction_done and (not st.session_state.streaming_running):
            ...
            st.session_state.start_stream_request = False  # Clear BEFORE starting to prevent re-entry
    ...
    # ---------------- Qwen refinement (MAIN thread; local Ollama) - FIXED ----------------
    if st.session_state.qwen_enabled:
        ...
        if (chunk_ready or force_ready) and st.session_state.full_transcript.strip():
            ...
            if st.session_state.qwen_inflight:
                log("[QWEN] Skipping: already inflight")
            elif now - st.session_state.qwen_last_call_ts < 10:  # 10s throttle (increased from 5)
                log(f"[QWEN] Skipping: throttled (last call {now - st.session_state.qwen_last_call_ts:.1f}s ago)")
            else:
                ...
                # Show spinner during Qwen processing
                with st.spinner("Qwen is refining translation..."):
                    try:
                        ...
                        log(f"[QWEN] Calling qwen_refine_translation with ar_len={len(ar_tail)}, draft_len={len(draft_tail)}")
                        refined = qwen_refine_translation(ar_tail, draft_tail, target_lang, qwen_log_path=st.session_state.qwen_log_path)
                        
                        if refined and len(refined) > 50:
                            st.session_state.full_translation_refined = refined
                            st.session_state.qwen_last_error = ""
                            log(f"[QWEN] SUCCESS: Refined len={len(refined)}")
                        else:
                            error_msg = f"Qwen returned empty or short response (len={len(refined)})"
                            st.session_state.qwen_last_error = error_msg
                            log(f"[QWEN] FAILED: {error_msg}")
                    except Exception as e:
                        msg = f"{type(e).__name__}: {str(e)}"
                        st.session_state.qwen_last_error = msg
                        log(f"[QWEN] EXCEPTION: {msg}")
                    finally:
                        st.session_state.qwen_inflight = False
                        log("[QWEN] Refinement complete, qwen_inflight=False")
